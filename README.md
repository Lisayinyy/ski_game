...
转弯刻滑、犁式刹车、跳跃落地时，板下会**腾起一团白色雪雾**——滑得越快、刃咬得越狠、
雪越深，雪雾越浓越高（`game.emitSpray` 按速度 × 刃角 × 深雪程度算发射量与初速，落地
用 `burstLanding` 打一圈粉雪爆）。粒子在 `fx.js` 的 `Spray` 里用一个小 shader 逐粒控制
大小与透明度，先快速鼓起再缓慢消散，像真实粉雪那样"炸开又飘落"。

> 关键：亮雪（反照率≈0.95）上**不能用 AdditiveBlending**——加色会被白雪冲平到看不见。
> 雪雾走 `NormalBlending` + 逐粒 alpha，才在雪面上读得出来。

### 黄金时刻氛围

不是六个雪场都顶着同一片正午蓝天。天空是一个三段渐变穹顶（`skyLow→skyMid→skyTop`）
加一颗会辉光的太阳，配色全部写在各雪场的 `palette` 里，于是氛围各不相同：

- **Aspen（阿斯本）**：午后金光——暖橙地平线爬向冷蓝天顶，低太阳把树和雪包的影子拉长。
- **Big Sky（大天空）**：日落粉金（alpenglow）——粉紫夜幕压着火烧云地平线，Lone Peak
  投出长长的玫瑰金影子。
- 其余四场保持清爽白天 / 阴天粉雪，做对比。

太阳压得越低，阴影越长、氛围越戏剧化。`scene.background` 用天顶色兜底，避免低太阳 +
大视场时天空穹顶顶端露出黑边。

### 大山质感：裸露岩壁 + 厚雪压枝的针叶林

雪道两侧不再是一排排孤零零的锥形树，而是按雪场难度铺出的真实高山地貌：

- **裸露岩石 / 悬崖岩层**：`world.js` 的 `buildCliff()` 堆叠四层错位的岩石岩层
  （两种岩色 `rock` / `rockDark = rock*0.82`），在低矮岩台上盖两块残雪。密度和体量按
  难度分级（`CLIFF` 表：绿道无岩、双黑道满岩），放在雪道外沉进 massif 里，离得近的登记为
  `rock` 障碍。于是 Palisades / Zermatt 这些黑道两侧是层叠的岩壁框着雪道，Deer Valley
  这种绿道则干干净净、一块岩石都没有。
- **厚雪压枝的针叶林**：`buildTree('spruce')` 每一层绿叶上都扣一顶又宽又厚的过垂雪冠
  （`ConeGeometry`），再挂两条被雪压弯下垂的雪枝、顶上一撮雪帽——像刚下过一场大雪，
  而不是随便撒几个绿三角。

岩壁密度和树的雪量都从 `difficulty` 推导，不引入新的雪场字段。
...
当前：**91/91 通过**，截图在 `qa/shots/`。覆盖内容包括：
...
- **低太阳 + 大视场会露出天空黑边**：太阳压低做黄金时刻氛围时，天空穹顶顶端偶尔会掉到
  远裁剪面之外，露出黑色清屏色的三角。给 `scene.background` 兜一层天顶色即可（天空球本身
  还要 `fog:false`，否则天顶会被雾染色）。
